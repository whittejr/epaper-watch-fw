...
uint8_t ssd1681_main(void) {
    ...
    res = ssd1681_set_driver_output(&gs_handle, SSD1681_BASIC_DEFAULT_DRIVER_OUTPUT_MUX,
                                    SSD1681_BASIC_DEFAULT_DRIVER_OUTPUT_GD, SSD1681_BASIC_DEFAULT_DRIVER_OUTPUT_SM,
                                    SSD1681_BASIC_DEFAULT_DRIVER_OUTPUT_TB);
    if (res != 0)
        return 1;

    res = ssd1681_set_data_entry_mode(&gs_handle, SSD1681_BASIC_DEFAULT_ADDRESS_MODE,
    ...
    res = ssd1681_set_display_update_control(&gs_handle, SSD1681_BASIC_DEFAULT_DISPLAY_CONTROL_RED,
    ...
    res = ssd1681_set_source_driving_voltage(&gs_handle, SSD1681_BASIC_DEFAULT_VSH1, SSD1681_BASIC_DEFAULT_VSH2,
    ...
    res = ssd1681_set_auto_write_red_ram(&gs_handle, SSD1681_BASIC_DEFAULT_AUTO_RED_STEP_HEIGHT,
                                         SSD1681_BASIC_DEFAULT_AUTO_RED_STEP_WIDTH);
    if (res != 0)
        return 1;

    /* set the default auto write black ram */
    res = ssd1681_set_auto_write_black_ram(&gs_handle, SSD1681_BASIC_DEFAULT_AUTO_BLACK_STEP_HEIGHT,
                                           SSD1681_BASIC_DEFAULT_AUTO_BLACK_STEP_WIDTH);
    if (res != 0)
        return 1;

    /* set the default hv ready detection */
    res = ssd1681_set_hv_ready_detection(&gs_handle, SSD1681_BASIC_DEFAULT_COOL_DOWN_DURATION,
    ...
    res = ssd1681_set_booster_soft_start(
        &gs_handle, (uint8_t)(SSD1681_BASIC_DEFAULT_PHASE1), (uint8_t)(SSD1681_BASIC_DEFAULT_PHASE2),
        (uint8_t)(SSD1681_BASIC_DEFAULT_PHASE3), (uint8_t)(SSD1681_BASIC_DEFAULT_DURATION));
    if (res != 0)
        return 1;
